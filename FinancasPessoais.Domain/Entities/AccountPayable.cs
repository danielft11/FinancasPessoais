...
namespace FinancasPessoais.Domain.Entities
{
    public class AccountPayable : BaseEntity
    {
        ...
        public AccountPayable(DateTime dueDate, decimal value, Guid categoryId, string description, string barCode, DateTime scheduleDate, string emails, string filePath)
        {
            ValidateDomain(dueDate, value, categoryId, description);
            BarCode = barCode;
        ...
        private void ValidateDomain(DateTime dueDate, decimal value, Guid categoryId, string description)
        {
